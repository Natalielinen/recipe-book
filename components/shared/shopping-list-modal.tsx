...
export const ShoppinListModal: React.FC<Props> = ({
    ...
    const cresteTextList = () => {
        const textList = ingredientsList?.map((ingredient) => {
            const amount =
                ingredient.amount -
                (inStockValues.find((value) => value.id === ingredient.id)?.value || 0);
    ...
    return (
        <Dialog
            ...
            onOpenChange={() => setShoppingListModal(false)}
        >
            <DialogContent
                className={cn(
            ...
            >
                <div>
                    {ingredientsList?.map((ingredient) => (
                        <div
                            key={ingredient.name}
                            className="flex justify-between my-4 flex-wrap"
                        >
                            <p className="mb-1 mt-2 font-bold w-[150px]">{ingredient.name}</p>
                            <div className="flex gap-10 items-center w-[300px]">
                                <Label htmlFor="email">{translation[lang as Lang].iHave}</Label>
                                <Input
                                    type="number"
                                    id="inStock"
                                    className="w-20"
                                    value={
                                        inStockValues.find((value) => value.id === ingredient.id)
                                            ?.value
                                    }
                                    onChange={(e) =>
                                        handleInStockChange(
                                            ingredient.id as number,
                            ...
                            <div className="flex gap-10 items-center w-[210px]">
                                <p className="p-0 m-0">{translation[lang as Lang].iNeed}</p>
                ...
                <Button variant="outline" onClick={cresteTextList}>
                    {translation[lang as Lang].createShoppingList}
                </Button>
            </DialogContent>
        </Dialog>
    );
