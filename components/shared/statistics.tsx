...
export const Statistics: React.FC<StatisticsProps> = ({ data }) => {
    ...
    const fetchRecipesInfo = async () => {
        ...
        setStatistics({
            recipesCount: response.reduce(
                (acc, item) => acc + item.recipes.length,
                0,
            ),
            recipesByCategory: response.map((el) => ({
                categoryName: el.nameKey,
                recipesCount: el.recipes.length,
            })),
        });
    ...
    return (
        <ul className="mt-10 border-l-muted-foreground">
            <li>
                {loading ? (
                    ...
                    >
                        {translation[lang as Lang].totalRecipes}:{" "}
                        <b>{statistics.recipesCount}</b>
                    </Link>
                )}
            </li>
            <li>
                {translation[lang as Lang].recipesByCategory}:{" "}
                {loading ? (
                    <Skeleton className="w-full h-[90px] rounded-2xl mb-1" />
                ) : (
                    <ul className="ml-4">
                        {statistics.recipesByCategory.map((el) => (
