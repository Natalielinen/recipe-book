...
export const updateRecipe = async (body: FormRecipe, id: number) => {
  const data = await axiosInstance.patch<FormRecipe>(
    ApiRoutes.RECIPE + `/${id}`,
    body
  );
...
export const getRecipeById = async (id: number) => {
  const { data } = await axiosInstance.get<FormRecipe>(
    ApiRoutes.RECIPE + `/${id}`
  );
...
export const search = async (query: string) => {
  const { data } = await axiosInstance.get<Recipe[]>(ApiRoutes.SEARCH_RECIPE, {
    params: { query },
  });
